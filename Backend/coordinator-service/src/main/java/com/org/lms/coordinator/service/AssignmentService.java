...
public class AssignmentService {
    ...
    public AssignmentResponse createAssignment(
        ...
        if (assignmentRepository
                .existsByCourseCourseIdAndTitleIgnoreCase(
                        courseId,
                        request.getTitle())) {

            throw new RuntimeException(
                    "Assignment already exists with title: "
                            + request.getTitle());
        }
    ...
    public AssignmentResponse updateAssignment(
        ...
        if (!assignment.getCourse()
            ...
            throw new IllegalArgumentException(
                    "Assignment does not belong to the specified course");
        }
    ...
    private void validateAssignmentDates(
        ...
        if (request.getAssignedDate() != null
                ...
                && request.getAssignedDate().isAfter(request.getDueDate())) {
        ...
        if (request.getMaximumSubmissionFileSize() != null
                && request.getMaximumSubmissionFileSize() <= 0) {

            throw new IllegalArgumentException(
                    "Maximum submission file size must be greater than zero");
        }

        if (course.getStartDate() != null
                && request.getAssignedDate() != null
                && request.getAssignedDate().isBefore(course.getStartDate())) {

            throw new IllegalArgumentException(
                    "Assigned date cannot be before course start date");
        }

        if (course.getEndDate() != null
                && request.getDueDate() != null
                && request.getDueDate().isAfter(course.getEndDate())) {

            throw new IllegalArgumentException(
                    "Due date cannot be after course end date");
        }
    }
    ...
    private AssignmentResponse mapToResponse(
        ...
        response.setAttachmentFileName(
                assignment.getAttachmentFileName());

        response.setAttachmentContentType(
                assignment.getAttachmentContentType());

        response.setAttachmentFileSize(
                assignment.getAttachmentFileSize());

        response.setAttachmentStoragePath(
                assignment.getAttachmentStoragePath());

        response.setMaximumSubmissionFileSize(
                assignment.getMaximumSubmissionFileSize());
